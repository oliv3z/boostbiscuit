...
namespace hotspot {
    ...
    typedef struct {
        const char* type_name;            // The type name containing the given field (example: "Klass")
        const char* field_name;           // The field name within the type           (example: "_name")
        const char* type_string;          // Quoted name of the type of this field (example: "Symbol*";

        // parsed in Java to ensure type correctness
        int32_t  is_static;               // Indicates whether following field is an offset or an address
    ...
    enum {
        ...
        JVM_ACC_LOOPS_FLAG_INIT = static_cast<int>(0x80000000),// The loop flag has been initialized
        ...
        JVM_ACC_IS_CLONEABLE_FAST = static_cast<int>(0x80000000),// True if klass implements the Cloneable interface and can be optimized in generated code
    ...
    struct method {
        void set_inlining(bool enabled);
        unsigned short* get_flags();
        access_flags* get_access_flags();
        void* get_i2i_entry();
    };
    ...
    struct thread {
        ...
        void set_thread_state(java_thread_state state);
        static int get_thread_state_offset();
    };

    struct frame {
        inline static int locals_offset = jvm_manager::get().get_client_brand() == ClientBrand::BADLION ? -56 : -48;  // -48 for java 8, -56 for java 17 (allegedly);
        void** get_locals();
        method* get_method();
    };

    vm_struct_entry* find_vm_struct_entry(const char* type_name, const char* field_name, bool is_static);

    void dump_vm_struct_entries();

    static constexpr int NO_COMPILE = JVM_ACC_NOT_C1_COMPILABLE | JVM_ACC_NOT_C2_COMPILABLE | JVM_ACC_NOT_C2_OSR_COMPILABLE | JVM_ACC_QUEUED;
}
